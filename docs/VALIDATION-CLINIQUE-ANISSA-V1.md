...
### ✅ SCELLÉ 2026-06-12 — lot sécurité grossesse / allaitement câblé et validé Anissa

Le P0 maternel est **câblé sur le chemin actif** (option (a)) et **validé par Anissa le
12 juin 2026**. Étapes :

- **P5.1** — `resolveMaternalState` (`_detector.fr.js`, exporté) : le détecteur lit
  désormais le format combiné in-app (`grossesseActuelle = "Grossesse"/"Allaitement"/
  "PostPartum"`) ET le legacy séparé (`"Oui"`), dispatch par valeur (piège anti-faux-
  positif allaitement≠grossesse verrouillé).
- **P0** — map déclarative `MATERNAL_SAFETY_FR/EN` injectée par `buildSafetyBlockFr` +
  `buildSafetyBlockEn` (calcul **avant** la garde précoce → cliente enceinte/allaitante
  sans allergie ni médicament reçoit quand même ses contraintes). Prouvé sur le chemin
  réel `composerBeta=false → buildSystemPromptFr`. Miroir EN local (zéro bleed FR).

**Formulations validées Anissa — 12 juin 2026 (approuvées telles qu'écrites) :**
- **Grossesse :** pas de jeûne (intermittent ni prolongé) ; pas de restriction calorique
  agressive ; vigilance vitamine A (forte dose / sources concentrées type foie) ;
  vigilance iode (sur- ET sous-dosage, avis praticienne).
- **Allaitement :** pas de restriction calorique agressive ; vigilance supplémentation
  (à valider avec la praticienne).
- **Étiquette « CONTRAINTES ABSOLUES » conservée** (décision Anissa : les vigilances
  vit. A / iode restent sous ce label).
- **Post-partum :** traité comme **cliente standard** par **décision Anissa** (plus un
  gap clinique) — état détecté, aucun bloc injecté en V1.

**Portée précise (pour qu'aucune session future ne croie « tout le système maternel est
actif ») :** le chemin actif porte les **contraintes maternelles condensées** (grossesse +
allaitement) + allergènes/intolérances/médicaments. Le **module profil composer complet**
(`GROSSESSE_MODULE_FR` + guardrails phrases interdites) reste **OPT-IN et dort** ; la
**détection mineur <18** reste à créer. Ces deux points sont hors P0.
